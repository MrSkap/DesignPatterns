...
Позволяет избежать привязки отправителя запроса и его получателя, так несколько получателей могут попытаться обработать запрос. Связывает получателей в цепочку, пока кто-то из них не обработает запрос.

Своебразная реализация в .net встречается при использовании событий, через аргументы которого передается свойства обработанности (Handled), если оно true, то запрос уже обработан и дургие получатели уведомления о событии не будут пытать его обработать.
Также в библитеке MediatR при использовании пайплайнов получается идентичная этому паттерну ситация: пока никто из обрабочков не примет запрос, они будут передовать его дальше по цепочке.

Когда использовать:
* когда не понятно, кто должен обработать запрос или количество обработчиков может поменяться, как и они сами
* когда логика обработки запроса может сильно отличаться в зависимости от внутренностей самого запроса. Городить switch-case или множество if будет неуместно и сложно
